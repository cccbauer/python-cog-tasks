...
# -*- coding: utf-8 -*-
"""
This experiment was created using PsychoPy2 Experiment Builder (v1.78.01), Tue Sep  3 13:33:58 2013
...
while continueRoutine and routineTimer.getTime() > 0:
    ...
    elif fix_stim.status == STARTED and t >= (0.0 + 3):
...
exp_loop = data.TrialHandler(nReps=8, method='random', 
...
for thisExp_loop in exp_loop:
    ...
    for thisTarget_loop in target_loop:
        ...
        for j in target_stim:
           if target_stim.count(j) == 1:
              target_correct=j
              break
        #print target_stim
        ...
        while continueRoutine and routineTimer.getTime() > 0:
            ...
            elif stim_text.status == STARTED and t >= (0.0 + 1.2):
            ...
            elif stim_response.status == STARTED and t >= (0 + 1.2):
                stim_response.status = STOPPED
            if stim_response.status == STARTED:
                theseKeys = event.getKeys(keyList=['1', '2', '3'])
                if len(theseKeys) > 0:  # at least one key was pressed
                    if stim_response.keys == []:  # then this was the first keypress
                        stim_response.keys = theseKeys[0]  # just the first key pressed
                        stim_response.rt = stim_response.clock.getTime()
                        # was this 'correct'?
                        if (stim_response.keys == str(target_correct)): stim_response.corr = 444
    ...
    # set up handler to look after randomisation of conditions etc
    ctrl_loop = data.TrialHandler(nReps=12, method='random', 
        extraInfo=expInfo, originPath=None,
        trialList=[None],
    ...
    for thisCtrl_loop in ctrl_loop:
        ...
        while continueRoutine and routineTimer.getTime() > 0:
            ...
            elif control_stim.status == STARTED and t >= (0.0 + 1.2):
            ...
            elif control_resp.status == STARTED and t >= (0.0 + 1.2):
            ...
            if control_resp.status == STARTED:
                ...
                if len(theseKeys) > 0:  # at least one key was pressed
                    ...
                    # was this 'correct'?
                    if (control_resp.keys == str(ctrl_correct)): control_resp.corr = 333
    ...
    # completed 12 repeats of 'ctrl_loop'
